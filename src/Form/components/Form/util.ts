...
import {
    ...
    IFieldCondition,
    IFieldConfig, IGenericField,
    IPatternCondition,
    IRangeCondition,
    IStringIndexableObject,
    IValueCondition,
    IValuesCondition
...
const matchValueCondition = (condition: IValueCondition, fieldValue: string | number) => {
    const {value} = condition;
    return value === fieldValue;
}

const matchValuesCondition = (condition: IValuesCondition, fieldValue: string | number) => {
    const {value} = condition;
    return value.includes(fieldValue);
}

const matchPatternCondition = (condition: IPatternCondition, fieldValue: string | number) => {
    const {value} = condition;
...
const matchRangeCondition = (condition: IRangeCondition, fieldValue: number) => {
    const {value} = condition;

    return fieldValue >= value[0] && fieldValue <= value[1];
}

export const matchFieldCondition = (condition: IFieldCondition, fieldValue: string | number) => {
    switch (condition.type) {
        ...
        case E_FIELD_CONDITION_TYPES.RANGE:
            return matchRangeCondition(condition as IRangeCondition, fieldValue as number);
...
export const applyConditions = (fieldValues: IStringIndexableObject<string | number>, conditions?: IStringIndexableObject<IFieldCondition>) => {
    if (conditions === undefined) {
        ...
        return {
            effect: (effect as unknown) as E_CONDITION_EFFECTS, isMet: Object.entries(rest)
                .map(([field, condition]) => matchFieldCondition(condition, fieldValues[field]))
                .every(c => c)
        }
    }
}

export const getDependentOnFields = (fieldConfigs: IStringIndexableObject<IGenericField<any, any>>) => {
